  ...
  async conversor(wikiText: string, attachments: any[] = []): Promise<string> {
    try {
      ...
      if (html === null || html === '' || html === '<None>') {
        ...
        // 1. Link with Title: [text|http://link]
        html = html.replace(
          /\[([^\|\]]+)\|(https?:\/\/[^\]]+)\]/g,
          '<a href="$2" style="cursor:pointer;">$1</a>',
        );

        // 2. Direct Link without Title: [http://link]
        html = html.replace(
          /\[(https?:\/\/[^\]]+)\]/g,
          '<a href="$1" style="cursor:pointer;">$1</a>',
        );
        ...
        if (imageMatches.length > 0) {
          ...
          const imagePromises = imageMatches.map(async (match) => {
            ...
            // todo Have to fix
            var hasSize: boolean = imageRef.split('|')[1]
              ? imageRef.split('|')[1].match(/width=/)
                ? true
                : false
              : false;
